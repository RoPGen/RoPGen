...
def alter_for_name(for_elems, author_for_names,typedef_names,func_s):
    ...
    for author_name in author_for_names:
        ...
        author_name_elem = author_name_elems[0] if len(author_name_elems)>0 else (author_name.xpath('src:init//src:argument_list//src:name',namespaces=ns)[0] if len(author_name.xpath('src:init//src:argument_list//src:name',namespaces=ns))>0 else None)

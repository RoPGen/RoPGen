...
class StyloFeaturesAbstract(StyloFeatures):
    ...
    def __init__(self, inputdata: typing.Union[str, list, dict],
        ...
        self._featurematrix, self._colnames, self._authors, self._iids, self._doclabels = self.read_data_from_source(inputdata,
                                                                                                    trainobject)
    ...
    def getdoclabels(self):
        if self.codestyloreference is None:
            return copy.deepcopy(self._doclabels)
        else:
            cmps = (self.codestyloreference.getdoclabels() != self._doclabels)
            assert np.sum(cmps) == 0
            return copy.deepcopy(self._doclabels)

...
class StyloUnigramFeatures(StyloFeaturesAbstract):
    ...
    def read_data_from_source(self, inputdata, trainobject):
        ...
        unigrammatrix, colnames, authors, iids, doclabels = self.__get_wordunigrams(verbose=False, inputdata=inputdata, trainobject=trainobject)
        return unigrammatrix, colnames, authors, iids, doclabels
    ...
    def __get_wordunigrams(self, verbose:bool, inputdata: str, trainobject: typing.Optional['StyloUnigramFeatures']) \
        ...
        if inputdata.endswith("cpp"):
            ...
            for pro in programmers:
                progfiles = [f for f in os.listdir(pro) if os.path.isfile(os.path.join(pro, f)) and (f.endswith("cpp") or f.endswith(".c"))]
        ...
        feature_matrix, iids, authors, doclabels = utils_authorship.sortmatrix(matrix1=sparse.csr_matrix(unigrammatrix), matrix1ids=np.array(documents_labels),
                                                                    matrix1authors=np.array(documents_authors), matrix1doclabels=np.array(documents_labels))
        ...
        return feature_matrix, feature_names, authors, iids, doclabels

...
def sortmatrix(matrix1: scipy.sparse.csr_matrix, matrix1ids: np.ndarray, matrix1authors: np.ndarray = None, matrix1doclabels=None)\
    ...
    # newmatrix = scipy.sparse.lil_matrix(matrix1.shape, dtype=np.float32)
    newauthors = np.copy(matrix1authors)
    newdoclabels = np.copy(matrix1doclabels)
    row_list = []

    for i in range(matrix1.shape[0]):
        matchix = np.where(newindex[i] == matrix1ids)[0][0]
        # newmatrix[i, :] = matrix1[matchix, :]
        row_list.append(matrix1[matchix, :])
        if matrix1authors is not None:
            newauthors[i] = matrix1authors[matchix]
        if matrix1doclabels is not None:
            newdoclabels[i] = matrix1doclabels[matchix]
    newmatrix = scipy.sparse.vstack(row_list)

    newmatrixsparse = scipy.sparse.csr_matrix(newmatrix, dtype=np.float32)

    return newmatrixsparse, newindex, newauthors, newdoclabels
